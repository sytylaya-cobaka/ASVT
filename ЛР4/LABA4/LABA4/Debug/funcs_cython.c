...
__PYX_EXTERN_C float fun_el(int, float); /*proto*/
...
float fun_el(int __pyx_v_k, float __pyx_v_x) {
  float __pyx_r;
  float __pyx_t_1;
  float __pyx_t_2;
  int __pyx_lineno = 0;
 ...
 *     return powf(x, -(<float>k)) / sinf((<float>k) * x)             # <<<<<<<<<<<<<<
*/
  __pyx_t_1 = powf(__pyx_v_x, (-((float)__pyx_v_k)));
  __pyx_t_2 = sinf((((float)__pyx_v_k) * __pyx_v_x));
...
*/
  ...
  /* function exit code */
  __pyx_L1_error:;
  __Pyx_AddTraceback("funcs.fun_el", __pyx_clineno, __pyx_lineno, __pyx_filename);
  __pyx_r = -1;
  __pyx_L0:;
  return __pyx_r;
}
...
__Pyx_RefNannySetupContext("PyInit_funcs", 0);
